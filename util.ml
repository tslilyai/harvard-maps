...
module type CRAWLER_SERVICES =
  sig
    ...
    val link_compare : link -> link -> Ordering.t
  ...
  struct
    ...
    let string_of_link l =
      if l.host = "" then "file://" ^ l.path
        ...
        l.host ^ ":" ^ (String.create l.port) ^ l.path
    ;;

    let link_compare (x:link) (y:link) : Ordering.t =
      match string_compare x.host y.host with
        | Equal ->
            (match int_compare x.port y.port with
               | Equal -> (match string_compare x.path y.path with
                          | Equal -> Equal
                          | ans -> ans)
               | ans -> ans)
        | ans -> ans
    ...
    let string_of_page p =
      ...
      let trunc s =
        if String.length s > 100 then
          (String.sub s ~pos:0 ~len:100) ^ "..."
        else s in
      (* concatenate the links and truncate them *)
      let ls = trunc (String.concat ~sep:";"
                        (List.map p.links ~f:string_of_link)) in
      (* concatenate the words and truncate them *)
      let ws = trunc (String.concat ~sep:";" p.words) in
    ...
    let strip_headers page =
      let rec find_two_newlines i =
        if i+2 < String.length page then
          match String.sub page ~pos:i ~len:2 with
            | "\n\n" -> Some (i+2)
            | "\r\n" ->
                if i+4 < String.length page then
                  (match String.sub page ~pos:(i+2) ~len:2 with
                     | "\r\n" -> Some (i+4)
      ...
      in
        match find_two_newlines 0 with
          | None -> page
          | Some i -> String.sub page ~pos:i ~len:(String.length page - i)
    ...
    let rec receive_message fd contents =
      let len = Unix.recv fd ~buf:buf ~pos:0 ~len:buf_len ~mode:[] in
        if len = 0 then String.concat ~sep:"" (List.rev contents)
        else receive_message fd ((String.sub buf ~pos:0 ~len:len)::contents)
    ;;
    ...
    let inet_fetch_url link : string option =
      let host_addr =
        try Unix.Inet_addr.of_string link.host
        with Failure _ ->
          (Unix.Host.getbyname_exn link.host).Unix.Host.addresses.(0) in
      let fd = Unix.socket ~domain:Unix.PF_INET ~kind:Unix.SOCK_STREAM ~protocol:0 in
      let _ = Unix.connect fd ~addr:(Unix.ADDR_INET (host_addr,link.port)) in
      let msg =
        "GET "^link.path^" HTTP/1.1\r\nHost: "^link.host^":"^
          (String.create link.port)^"Connection: close\r\n\r\n" in
      let i = Unix.send fd ~buf:msg ~pos:0 ~len:(String.length msg) ~mode:[] in
    ...
    let file_fetch_url link : string option =
      ...
      let rec receive_message chan contents =
        let len = input chan buf 0 buf_len in
          if len = 0 then String.concat ~sep:"" (List.rev contents)
          else receive_message chan ((String.sub buf ~pos:0 ~len:len)::contents) in
      let result = receive_message chan [] in
        In_channel.close chan ; Some result
    ...
    let fetch_url link : string option =
      try
        if (List.exists valid_suffixes ~f:(Filename.check_suffix link.path)) then
          ...
          if pos < len then
            let e = find_end 0 in
              if e = 0 then loop (pos+1) words else
                loop (pos+e) ((String.sub page ~pos:pos ~len:e)::words)
          else words
      in loop 0 []
    ...
    let href_of_link l =
      let suffix str start =
        ...
        String.sub str ~pos:start ~len:(len - start)
      in
      if l.host = "" then
